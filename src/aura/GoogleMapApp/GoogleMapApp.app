<aura:application controller="GoogleMap" extends="force:slds"> 
    ...
    <aura:attribute name="mapOptions" type="Object" default='{"zoom": 1}' />
    <aura:attribute name="mapOptionsCenter" type="Object" />
    <aura:attribute name="mapData" type="Object[]" />
    
    
    <aura:if isTrue="{!v.acc.length > 0}">
	    <c:GoogleMap acc="{!v.acc}" mapOptions="{!v.mapOptions}" mapOptionsCenter="{!v.mapOptionsCenter}" mapData="{!v.mapData}" />
    </aura:if>
</aura:application>