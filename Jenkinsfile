                    ...
                    steps {
                       checkout scm
						...
						script { 
								...
								rc = sh returnStatus: true, script: "sfdx force:auth:logout --targetusername ${SFDC_HUB_USERNAME} -p & sfdx force:auth:jwt:grant --clientid ${CONSUMER_KEY} --username ${SFDC_HUB_USERNAME} --jwtkeyfile \"${JWT_KEY}\" -d --instanceurl ${SFDC_HOST}"
								if (rc != 0){
